...
For a 21px char height and 16px font, with no key hints:
  ...
  (let* ((char-height (map-elt header-model :font-height))
         ...
         (image-height (* 2 char-height))
         (title-height (* 3 char-height))
    ...
    `((:char-height . ,char-height)
      ...
      (:total-height . ,(+ title-height row-spacing top-padding-height bottom-padding-height))
      (:icon-x . ,icon-x)
      (:icon-y . ,(- icon-text-y font-size))
      (:icon-text-x . ,(+ icon-x image-height 10))
      (:icon-text-y . ,icon-text-y)
      (:key-hints-x . ,icon-x)
      (:key-hints-y . ,(+ title-height font-size row-spacing)))))
